...
# 教师网络带有增强的fed_mu
# 第二种增强，教师网络和学生网络都轮流吃原始数据和增强数据
import copy
import math
from tensorboardX import SummaryWriter
import numpy as np
from torch.utils.data import Subset, DataLoader
from methods.client.local_train import LocalMutualAug2, LocalMutualAug3, LocalMutualMpl
...
def fed_mutual(args, testset, part_data):
    ...
    trainset, aug_dataset = mpl_tool.get_train_set_aug2(args)
    # 所有已经分好组的训练集和测试集
    ...
    for item in range(args.round_num):
        ...
        for k in range(args.clients_per_round):
            ...
            # 训练每个选到的客户端
            local = LocalMutualMpl(args, train_loaders[idx_users[k]], train_loader_aug[idx_users[k]], lr)
        ...
        print(f'FedMulAug2 Round {item} Accuracy on global test set: {global_acc}%')
