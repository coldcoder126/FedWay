...
#         for k in idx_users:
#             # 训练每个选到的客户端
#             local = LocalTrainMAS(args, train_sets[k], lr)
...
def fed_mas(args, part_data):
    ...
    options['model'] = args.model
    model = md.choose_model(options)
    model, freeze_layers = create_freeze_layers(model, 2)
    model = init_reg_params(model, freeze_layers)
    model = model.to(device)
    lr = args.lr
    for item in range(1, args.round_num + 1):
        lr = tool.getLr(lr, item)
        ...
        for k in idx_users:
            ...
            global_model = copy.deepcopy(model)
            global_model = init_reg_params_across_tasks(global_model)
            param, loss = local.train_mas(global_model)
            param = consolidate_reg_params(param)
            local_acc = tool.global_test(param,test_loader)
            print(f"  FedMas Round {item} client {k} Acc:{local_acc}")
            selected_params.append(tool.get_flat_params_from(param.parameters()))
        ...
        # 每一轮训练完，测试全局模型在全局测试集上的表现
        global_acc = tool.global_test(model, test_loader)
        print(f'FedMas Round:{item} lr:{lr} clients:{idx_users} global_acc:{global_acc}%')
        writer.add_scalars('Loss/Epoch',
                           {'All Data': global_acc}, item)
