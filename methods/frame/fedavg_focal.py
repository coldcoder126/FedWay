...
# 实现fedavg算法
def fedavg_focal(args, part_data):
    train_set, test_set = tool.get_data_set(args, True)
    path = tool.mk_path(args)
    writer_file = f"fedavg_focal-{args.dataset}-clientNum{args.client_num}-dir{args.alpha}-seed{args.seed}-lr{args.lr}"
    ...
    for item in range(1, args.round_num + 1):
        ...
        for k in idx_users:
            ...
            param, loss = local.train_focal(global_model)
            local_acc = tool.global_test(param,test_loader)
            print(f"  FedAvg_focal Round {item} client {k} Acc:{local_acc}")
        ...
        print(f'FedAvg_focal Round:{item} lr:{lr} clients:{idx_users} global_acc:{global_acc}%')
