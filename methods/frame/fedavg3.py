...
def fedavg(args, trainset, testset, part_data):
    ...
    server_model = smd.RNN(gm_shape,gm_shape,args.client_num)

    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, server_model.parameters()), lr=0.01)
    server_model.train()
    ...
    for item in range(args.round_num):
        ...
        # 每轮随机选择部分客户端
        np.random.seed(item)
        idx_users = np.random.choice(range(args.client_num), 1, replace=False)
        ...
        if item>0:
            ...
            lm_t = torch.cat([param.data.view(-1,) for param in selected_layer[0]])
            # param_list.append(gm_t)
            # for c in selected_layer:
            #     param_list.append(torch.cat([param.data.view(-1,) for param in c]))
            # selected_layer.clear()
            # 将收集的tensor按dim=1拼接起来
            # layers = torch.stack(param_list).to(device)
            ...
            optimizer.zero_grad()
            output = server_model(gm_t,lm_t,selected_client_onehot)
        ...
        if idx_users[0] in client_private_params.keys():
            private_model = client_private_params[idx_users[0]]
        else:
        ...
        local = LocalTrain(args, train_loaders[idx_users[0]],lr)
        global_model = copy.deepcopy(gm_new)

        net, acc = local.train2(global_model)
        param_cp = copy.deepcopy(net)
        client_private_params[idx_users[0]] = param_cp
        ...
        selected_data_num.append(train_loaders[idx_users[0]].sampler.num_samples)

        accs.append(acc)
        # 拿到客户端的真实预测后，更新权重网络
        if item>0:
            criteria = nn.MSELoss(reduction='sum')
            label = torch.stack(accs)*100
            ...
            torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=1000, norm_type=2)
            optimizer.step()
