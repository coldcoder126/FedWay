...
# 实现scaffold算法
def scaffold(args, trainset, testset, part_data):
    path = tool.mk_path(args)
    writer_file = f"scaffold-{args.dataset}-clientNum{args.client_num}-dir{args.alpha}-seed{args.seed}-lr{args.lr}"
    ...
    for item in range(args.round_num):
        ...
        for k in range(args.clients_per_round):
            ...
            param, loss, temp_c = local.train_scaffold(global_model,global_c)
        ...
        global_c = sum(temp_c)/len(temp_c)
        temp_c.clear()
        ...
        print(f'Scaffold Round {item} Accuracy on global test set: {global_acc}%')
